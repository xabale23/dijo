-- ============================================================
-- DIJO
-- Migration: 022a_initial_order_status_history.sql
--
-- Purpose:
-- Record the initial pending lifecycle event for orders created
-- through the secure create_order() checkout RPC.
--
-- History semantics:
--   NULL -> pending = order successfully created
--
-- This replaces create_order() without changing its public
-- signature, checkout validation, pricing authority, or limits.
-- ============================================================
...
begin
    ...
    -- ========================================================
    -- 3. REQUEST VALIDATION + ABUSE LIMITS
    -- ========================================================
    ...
    if char_length(coalesce(p_delivery_address, '')) > 500 then
        raise exception 'Delivery address cannot exceed 500 characters';
    end if;

    if char_length(coalesce(p_customer_notes, '')) > 1000 then
        raise exception 'Customer notes cannot exceed 1000 characters';
    end if;
    ...
    loop
        ...
        if jsonb_array_length(
            ...
            raise exception
                'An order item cannot contain more than 50 option selections';
        end if;
    ...
    -- ========================================================
    -- 10. INITIAL ORDER LIFECYCLE EVENT
    -- ========================================================
    --
    -- The order has now passed all checkout validation and its
    -- authoritative totals/items/options have been persisted.
    --
    -- NULL -> pending represents successful order creation.
    -- Because this runs inside the same transaction, a failure
    -- anywhere in create_order() leaves neither an incomplete
    -- order nor a false lifecycle event.
    -- ========================================================
...
-- Preserve hardened ownership and execution boundary explicitly.
